...
describe('querySessionsByDate', () => {
  ...
  it('maps fields correctly', () => {
    ...
    expect(sess1.title).toBe('Fix auth bug');
    expect(sess1.timeCreated).toBe(new Date('2026-02-20T10:00:00Z').getTime());
    db.close();
  });
});

describe('queryMessagesBySessionIds', () => {
  it('returns only assistant messages with tokens', () => {
    const db = openDb(dbPath);
    const messages = queryMessagesBySessionIds(db, ['sess-001']);
    // sess-001 has 2 assistant + 1 user; only assistant with tokens returned
    expect(messages.length).toBe(2);
    expect(messages.every(m => m.role === 'assistant')).toBe(true);
    db.close();
  });

  it('parses token fields from JSON data', () => {
    const db = openDb(dbPath);
    const messages = queryMessagesBySessionIds(db, ['sess-001']);
    const msg1 = messages.find(m => m.id === 'msg-001')!;
    expect(msg1.modelId).toBe('claude-sonnet-4-6');
    expect(msg1.providerId).toBe('anthropic');
    ...
    expect(msg1.totalTokens).toBe(390);
    db.close();
  });
...
describe('aggregateByModel', () => {
  it('groups messages by model and sums tokens', () => {
    ...
    expect(claude.totalTokens).toBe(390);
    ...
    expect(gpt.totalTokens).toBe(480);
